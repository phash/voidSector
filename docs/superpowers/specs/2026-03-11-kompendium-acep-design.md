...
Parser erkennt `![...](img:pfad)` und rendert:
```tsx
<div className="compendium-img-block">
  <img src={`/compendium/${pfad}.png`} alt={caption} />
...
### Modifizieren
- `packages/client/src/data/compendium.ts` — neue Kategorie, 2 neue Artikel, 3 Updates, 2 neue Fallstudien
...
| Feature | Test |
|---|---|
| `'acep'` in CompendiumCategory | Unit: compendium-data.test.ts — Kategorie vorhanden |
| Neue Artikel vorhanden | Unit: compendium-data.test.ts — acep-monitor, acep-slots auffindbar |
| `img:`-Marker rendert `<img>` | Unit: CompendiumOverlay — renderBody mit `![cap](img:test)` |
| Keine broken seeAlso-Links | Unit: compendium-data.test.ts — alle seeAlso-IDs existieren |
