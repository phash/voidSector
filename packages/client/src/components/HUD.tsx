...
export function StatusBar() {
  ...
  const hdSecondsToFull = hyperdrive && !hdFull && hyperdrive.regenPerSecond > 0
    ...
    <div style={{
      ...
      {hyperdrive && (
        <>
          <span style={{ color: 'var(--color-dim)' }}>|</span>
          <span style={{
            color: displayCharge <= 0 ? '#FF3333' : displayCharge < hyperdrive.maxCharge * 0.2 ? '#FF6644' : '#00CCFF',
          }}>
            HYPER: {displayCharge}/{hyperdrive.maxCharge}
            {' '}<SegmentedBar current={displayCharge} max={hyperdrive.maxCharge} width={8} />
          </span>
          <span style={{ fontSize: '0.75rem', color: 'var(--color-dim)' }}>
            {hyperdrive.regenPerSecond}/s {hdFull ? <span style={{ color: '#00FF88' }}>FULL</span> : `FULL ${hdSecondsToFull}s`}
          </span>
        </>
      )}
      <span style={{ color: 'var(--color-dim)' }}>|</span>
