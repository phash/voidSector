...
describe('FactionConfigService', () => {
  ...
  it('loads faction configs on init', async () => {
    mockGetAllFactionConfigs.mockResolvedValue([
      ...
      { faction_id: 'consortium', home_qx: -10, home_qy: -20, expansion_rate: 10, aggression: 0.4, expansion_style: 'sphere', active: true },
    ]);
    await service.init();
  ...
  it('distanceTo calculates euclidean distance from faction home', async () => {
    mockGetAllFactionConfigs.mockResolvedValue([
      { faction_id: 'kthari', home_qx: 0, home_qy: 0, expansion_rate: 5, aggression: 2.0, expansion_style: 'sphere', active: true },
    ]);
    await service.init();
