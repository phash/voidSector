...
export async function getQuadrant(qx: number, qy: number): Promise<QuadrantData | null> {
  const { rows } = await query<any>('SELECT * FROM quadrants WHERE qx = $1 AND qy = $2', [qx, qy]);
...
export async function addPlayerKnownQuadrant(
  playerId: string,
  qx: number,
  qy: number,
): Promise<void> {
...
export async function addPlayerKnownQuadrantsBatch(
  playerId: string,
  quadrants: Array<{ qx: number; qy: number }>,
): Promise<void> {
