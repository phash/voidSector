...
import { QUADRANT_SIZE, QUADRANT_NAME_MIN_LENGTH, QUADRANT_NAME_MAX_LENGTH, WORLD_SEED } from '@void-sector/shared';
import type { QuadrantConfig, QuadrantData } from '@void-sector/shared';
import { getQuadrant, upsertQuadrant, addPlayerKnownQuadrant, quadrantNameExists, updateQuadrantName } from '../db/quadrantQueries.js';
...
const SYLLABLES = [
  'al', 'an', 'ar', 'as', 'at', 'ax', 'ba', 'be', 'bi', 'bo',
  'ca', 'ce', 'ci', 'co', 'da', 'de', 'di', 'do', 'el', 'en',
  'er', 'es', 'ex', 'fa', 'fi', 'ga', 'go', 'ha', 'he', 'hi',
  'in', 'ir', 'is', 'ka', 'ke', 'ki', 'ko', 'la', 'le', 'li',
  'lo', 'lu', 'ma', 'me', 'mi', 'mo', 'mu', 'na', 'ne', 'ni',
  'no', 'nu', 'or', 'os', 'pa', 'pe', 'pi', 'ra', 're', 'ri',
  'ro', 'ru', 'sa', 'se', 'si', 'so', 'su', 'ta', 'te', 'ti',
  'to', 'tu', 'un', 'ur', 'us', 'va', 've', 'vi', 'vo', 'xa',
  'xe', 'ya', 'za', 'ze', 'zi', 'zo',
];
...
export async function getOrCreateQuadrant(qx: number, qy: number, discoveredByPlayerId?: string): Promise<QuadrantData> {
  let quadrant = await getQuadrant(qx, qy);
  if (quadrant) return quadrant;

  const config = generateQuadrantConfig(qx, qy);
  quadrant = {
    qx, qy, seed: config.seed,
