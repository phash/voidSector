...
function parseCommand(text, line, errors) {
    const fly = text.match(/^fly\s+(-?\d+):(-?\d+)$/);
    if (fly)
        return { type: 'fly', x: Number(fly[1]), y: Number(fly[2]), line };
    if (text === 'scan')
        return { type: 'scan', line };
    if (text === 'mine' || text === 'mine until full')
        return { type: 'mine', mode: 'until_full', amount: 0, line };
    const mineN = text.match(/^mine\s+(\d+)$/);
    if (mineN)
        return { type: 'mine', mode: 'amount', amount: Number(mineN[1]), line };
    if (text === 'sell all')
        return { type: 'sell', target: 'all', line };
    const sellR = text.match(/^sell\s+(\w+)$/);
    if (sellR) {
        const r = sellR[1];
        if (SELLABLE_RESOURCES.includes(r))
            return { type: 'sell', target: r, line };
...
function parseBlock(lines, cursor, indent, errors) {
    const stmts = [];
...
function parseStatement(lines, cursor, indent, errors) {
    const ln = lines[cursor.i];
    const text = ln.text;
    if (text === 'repeat:') {
        cursor.i++;
        ...
        if (body.length === 0)
            errors.push({ line: ln.line, message: '`repeat` braucht einen eingerückten Block.' });
    ...
    if (repN) {
        ...
        const body = parseBlock(lines, cursor, indent + 1, errors);
        if (body.length === 0)
            errors.push({ line: ln.line, message: '`repeat N times` braucht einen eingerückten Block.' });
    ...
    if (ifm) {
        ...
        if (thenBlock.length === 0)
            errors.push({ line: ln.line, message: '`if` braucht einen eingerückten Block.' });
        let otherwise = null;
        if (cursor.i < lines.length && lines[cursor.i].indent === indent && lines[cursor.i].text === 'else:') {
            ...
            if (otherwise.length === 0)
                errors.push({ line: elseLn.line, message: '`else` braucht einen eingerückten Block.' });
        }
...
export function parseCondition(text, line, errors) {
    let negate = false;
    let body = text.trim();
    ...
    if (body === 'resources')
        return { kind: 'resources', negate };
    if (body === 'full')
        return { kind: 'full', negate };
    if (body === 'empty')
        return { kind: 'empty', negate };
    if (body === 'station')
        return { kind: 'station', negate };
    const fuel = body.match(/^fuel\s*<\s*(\d+)$/);
    if (fuel)
        return { kind: 'fuel_lt', value: Number(fuel[1]), negate };
    const at = body.match(/^at\s+(-?\d+):(-?\d+)$/);
    if (at)
        return { kind: 'at', x: Number(at[1]), y: Number(at[2]), negate };
